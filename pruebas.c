...
nodo_t* nodo_inicio_despues_de_desencolar(lista_t* cola, int cantidad){
	int i = 0;
	nodo_t* nodo_actual, *nodo_a_devolver;
	
	nodo_actual = cola->nodo_inicio;
	while(i < cantidad && nodo_actual){

		if((i + 1) == cantidad)
			nodo_a_devolver = nodo_actual->siguiente;
...
void probar_insertar_normal_lista(int* valor){
	...
	reiniciar_lista(lista, lista_borrar);
...
void probar_apilar_pila(int* valor){
	...
	ptda_afirmar(lista_apilar(NULL, &(valor[0])) == ERROR, "No se puede apilar en una pila no creada");
	
	retorno = insertar_elementos_lista(valor, pila, 1, lista_apilar);
	ptda_afirmar(retorno == EXITO, "Se puede apilar en una pila vacía");
	ptda_subafirmar((retorno == EXITO) && (1 == lista_elementos(pila)), "La cantidad de elementos de la pila es correcta");
	...
	reiniciar_lista(pila, lista_desapilar);
	retorno = insertar_elementos_lista(valor, pila, CANT_INSERCION, lista_apilar);	
	ptda_afirmar(retorno == EXITO, "Se puede insertar "STRING_CANT_INSERCION" elementos en la pila");
	ptda_subafirmar((CANT_INSERCION) == lista_elementos(pila), "La cantidad de elementos de la pila es correcta");
	inserto_bien = se_inserto_bien(pila, valor, CANT_INSERCION, 0, se_puede_seguir_insertar_normal, es_valor_insertar_normal);
	ptda_subafirmar_fin(inserto_bien, "El valor de los "STRING_CANT_INSERCION" elementos apilados es correcto");
...
void probar_desapilar_pila(int* valor){
	...
	ptda_afirmar(lista_desapilar(NULL) == ERROR, "No se puede desapilar una pila no creada");

	ptda_afirmar(lista_desapilar(pila) == ERROR, "No se puede desapilar en una pila vacía");
	
	insertar_elementos_lista(valor, pila, 1, lista_apilar);
	retorno = lista_desapilar(pila);
	ptda_afirmar(retorno == EXITO, "Se puede desapilar el unico elemento en una pila");
	ptda_subafirmar((retorno == EXITO) && (0 == lista_elementos(pila)), "La cantidad de elementos de la pila es correcta");
	...
	insertar_elementos_lista(valor, pila, CANT_INSERCION, lista_apilar);
	nodo_inicio_inicial = pila->nodo_inicio;
	retorno = borrar_muchos(pila, lista_desapilar);
	ptda_afirmar(retorno == EXITO, "Se puede desapilar "STRING_CANT_BORRAR" elementos de la pila");
	ptda_subafirmar(retorno == EXITO && (CANT_INSERCION - CANT_BORRAR) == lista_elementos(pila), "La cantidad de elementos de la pila es correcta");
	...
	while(lista_elementos(pila) != 0 && retorno == EXITO)
		retorno = lista_desapilar(pila);
	ptda_afirmar(retorno == EXITO, "Se puede desapilar todos los elementos de la pila");
	ptda_subafirmar_fin(retorno == EXITO && lista_vacia(pila), "La pila quedó vacía correctamente");
...
void probar_obtener_tope_pila(int* valor){
	...
	ptda_afirmar(!lista_tope(NULL), "No se puede obtener el tope de una pila no creada");

	ptda_afirmar(!lista_tope(pila), "No se puede obtener el tope de una pila vacía");
...
void probar_encolar_cola(int* valor){
	int retorno;
	bool inserto_bien = true;
	lista_t* cola;
	...
	ptda_nuevo_subgrupo("Encolar - cola");
	
	ptda_afirmar(lista_encolar(NULL, &(valor[0])) == ERROR, "No se puede encolar en una cola no creada");
	
	retorno = insertar_elementos_lista(valor, cola, 1, lista_encolar);
	ptda_afirmar(retorno == EXITO, "Se puede encolar en una cola vacía");
	ptda_subafirmar((retorno == EXITO) && (1 == lista_elementos(cola)), "La cantidad de elementos de la cola es correcta");
	ptda_subafirmar((retorno == EXITO) && lista_primero(cola) == &(valor[0]), "El valor del elemento encolado en la cola es correcto");
	ptda_subafirmar_fin((retorno == EXITO) && ((cola->nodo_inicio) == (cola->nodo_fin)) && (cola->nodo_inicio), "El inicio y fin de la cola son correctos");

	reiniciar_lista(cola, lista_desencolar);
	retorno = insertar_elementos_lista(valor, cola, CANT_INSERCION, lista_encolar);	
	ptda_afirmar(retorno == EXITO, "Se puede encolar "STRING_CANT_INSERCION" elementos en la cola");
	ptda_subafirmar((CANT_INSERCION) == lista_elementos(cola), "La cantidad de elementos de la cola es correcta");
	inserto_bien = se_inserto_bien(cola, valor, CANT_INSERCION, 0, se_puede_seguir_insertar_normal, es_valor_insertar_normal);
	ptda_subafirmar_fin(inserto_bien, "El valor de los "STRING_CANT_INSERCION" elementos encolados en la cola es correcto");

	lista_destruir(cola);
}

void probar_desencolar_cola(int* valor){
	int retorno;
	nodo_t *nodo_inicio_final, *nodo_fin_final;
	lista_t* cola;

	cola = lista_crear();

	ptda_nuevo_subgrupo("Desencolar - cola");

	ptda_afirmar(lista_desencolar(NULL) == ERROR, "No se puede desencolar una cola no creada");

	ptda_afirmar(lista_desencolar(cola) == ERROR, "No se puede desencolar una cola vacía");

	insertar_elementos_lista(valor, cola, 2, lista_encolar);
	nodo_inicio_final = (cola->nodo_inicio)->siguiente;
	nodo_fin_final = cola->nodo_fin;
	retorno = lista_desencolar(cola);
	ptda_afirmar(retorno == EXITO, "Se puede desencolar 1 elemento de una cola con 2 elementos");
	ptda_subafirmar(retorno == EXITO && 1 == lista_elementos(cola), "La cantidad de elementos de la cola es correcta");
	ptda_subafirmar_fin(retorno == EXITO && nodos_lista_validos(cola, nodo_inicio_final, nodo_fin_final), "Los elementos de la cola están correctamente ordenados");

	retorno = lista_desencolar(cola);
	ptda_afirmar(retorno == EXITO, "Se puede desencolar el unico elemento en una cola");
	ptda_subafirmar((retorno == EXITO) && (0 == lista_elementos(cola)), "La cantidad de elementos de la cola es correcta");
	ptda_subafirmar_fin((retorno == EXITO) && (cola->nodo_fin == NULL) && (cola->nodo_inicio == cola->nodo_fin), "El inicio y fin de la cola son correctos");

	insertar_elementos_lista(valor, cola, CANT_INSERCION, lista_encolar);
	nodo_inicio_final = nodo_inicio_despues_de_desencolar(cola, CANT_BORRAR);
	nodo_fin_final = cola->nodo_fin;
	retorno = borrar_muchos(cola, lista_desencolar);
	ptda_afirmar(retorno == EXITO, "Se puede desapilar "STRING_CANT_BORRAR" elementos de la cola");
	ptda_subafirmar(retorno == EXITO && (CANT_INSERCION - CANT_BORRAR) == lista_elementos(cola), "La cantidad de elementos de la cola es correcta");
	ptda_subafirmar_fin(retorno == EXITO && nodos_lista_validos(cola, nodo_inicio_final, nodo_fin_final), "Los elementos de la cola están correctamente ordenados");

	while(lista_elementos(cola) != 0 && retorno == EXITO)
		retorno = lista_desencolar(cola);
	ptda_afirmar(retorno == EXITO, "Se puede desencolar todos los elementos de la cola");
	ptda_subafirmar_fin(retorno == EXITO && lista_vacia(cola), "La cola quedó vacía correctamente");

	lista_destruir(cola);
}

void probar_obtener_primero_cola(int* valor){
	void* elemento;
	lista_t* cola;

	cola = lista_crear();

	ptda_nuevo_subgrupo("Obtener primero - cola");

	ptda_afirmar(!lista_primero(NULL), "No se puede buscar el primer elemento de una cola no creada");

	ptda_afirmar(!lista_primero(cola), "No se puede buscar el primer elemento de una cola vacía");

	insertar_elementos_lista(valor, cola, 1, lista_encolar);
	elemento = lista_primero(cola);
	ptda_afirmar(elemento, "Se puede buscar el primer elemento de una cola con 1 elemento");
	ptda_subafirmar_fin(elemento == &(valor[0]), "El valor del elemento buscado en la cola es correcto");
	
	reiniciar_lista(cola, lista_desencolar);
	insertar_elementos_lista(valor, cola, CANT_INSERCION, lista_encolar);
	elemento = lista_primero(cola);
	ptda_afirmar(elemento, "Se puede obtener el primer elemento de una cola con muchos elementos");
	ptda_subafirmar_fin(elemento == &(valor[0]), "El valor del elemento buscado en la cola es correcto");
...
void probar_cola(){
	...
	ptda_nuevo_grupo("Pruebas: operaciones");
	probar_encolar_cola(valor);
	probar_desencolar_cola(valor);
	probar_obtener_primero_cola(valor);
...
int main(){
	...
	ptda_nueva_categoria("Pruebas cola");
	probar_cola();
